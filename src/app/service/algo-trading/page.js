...
const Point = styled(Typography)({
  ...
  alignItems: "flex-start", // To align icon and text to top of text line
});
...
const InfoCard = styled(Box)(({ theme }) => ({
  ...
  "&:hover": {
    boxShadow: `0 20px 40px rgba(73, 50, 107, 0.4)`, // Replicated shadow, slightly stronger for visual impact
    // Replicated gradient background on hover (if any, not explicitly defined in the selection)
  },
  "&::before": {
    // Replicated pseudo-element
    content: '""',
    position: "absolute",
    top: 0,
    left: 0,
...
const AlgoTrading = ({ serviceName = "ALGO TRADING" }) => {
  // Default serviceName for consistency
      ...
      <Box sx={{ backgroundColor: "#f9f3fe", py: { xs: 4, md: 6 } }}>
        <Container maxWidth="lg">
            ...
            <Grid item xs={12} sm={8} md={8}>
              ...
              >
                ...
                <Box sx={{ p: 0, marginTop: "10px" }}>
                  {" "}
                  {/* Removed padding from box around points, added mt */}
                  <Point>
                    <RadioButtonCheckedIcon
                      sx={{
                        mr: 1, // Material-UI spacing unit (8px)
                        fontSize: { xs: "18px", sm: "20px" }, // Adjusted size for better alignment
                        color: "#49326b", // Ensure color matches
                      }}
                    />
            ...
            >
              <Box sx={{ display: "flex", justifyContent: "center" }}>
                <Image
                  ...
                  priority
                  placeholder="blur"
                  blurDataURL="data:image/png;base64,iVBORw0KGgoAAAANSUhCAAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAErgJ9aB4zYgAAAABJRU5ErkJggg=="
          ...
          >
            ...
            {ALGOS.map((algo, index) => (
              <Grid item xs={12} sm={6} md={4} key={index}>
                {" "}
                {/* Changed sm to 6 for 2 columns on small screens, md to 4 for 3 columns on medium */}
                <InfoCard>
                  <Typography
                    variant="h6" // Semantic variant
                    component="h3" // Semantic component
                    fontWeight="bold"
                    color="#49326b"
                    sx={{
                      fontSize: { xs: "0.9rem", sm: "1.1rem" }, // Responsive font size
                      textAlign: "center", // Ensures text is centered within its full width
                      width: "100%", // Explicitly ensure it takes full width
                      display: "block", // Ensures block-level behavior for textAlign
                      mb: 1.5, // Margin-bottom for spacing between title and points
                    }}
                  >
                    {algo.title}
                  </Typography>
                  ...
                  <Box
                    sx={{
                      ...
                      mt: 1, // Space between title and points
                    }}
                  >
          ...
          <Box sx={{ mt: 6, display: "flex", justifyContent: "center" }}>
            ...
            {/* Assuming GetMoreButton component exists */}
